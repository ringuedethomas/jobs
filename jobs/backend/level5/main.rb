...
def dispatchPricePerActor(item)
	driverPrice = item['price'] + item['options']['deductible_reduction']
	actions = Array.new
	driver = Hash[
		'who'		=> 'driver',
		'type'		=> 'debit',
	...
	actions << driver
	ownerPrice = item['price'] - (item['price'] * 30) / 100
	owner = Hash[
		'who'		=> 'owner',
		'type'		=> 'credit',
		'amount'	=> ownerPrice
	...
	insurance = Hash[
		'who'		=> 'insurance',
		'type'		=> 'credit',
		'amount'	=> item['commission']['insurance_fee']
	]
	actions << insurance
	assistance = Hash[
		'who'		=> 'assistance',
		'type'		=> 'credit',
		'amount'	=> item['commission']['assistance_fee']
	]
	actions << assistance
	drivyPrice = item['commission']['drivy_fee'] + item['options']['deductible_reduction']
	drivy = Hash[
		'who'		=> 'drivy',
		'type'		=> 'credit',
		'amount'	=> drivyPrice
...
def calculateRentalsPrice(rentalsData, carsData)
	...
	rentalsData.each do |index, rental|
		...
		actions = dispatchPricePerActor item
